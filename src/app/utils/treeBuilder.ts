...
export class TreeBuilder<T> {
    ...
    public build(entities: T[]): TreeNode<T>[] {
        ...
        const groupedByParent = this.groupByParent(entities);
    ...
    private groupByParent(entities: T[]): Map<string, T[]> {
        ...
        for (const entity of entities) {
            const parentKey = String(this.mapper.getParentId(entity) ?? 'root');
    ...
    private mapToNodes(
        ...
        const children = grouped.get(String(parentId ?? 'root')) || [];
