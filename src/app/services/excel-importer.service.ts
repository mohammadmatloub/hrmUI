...
export class ExcelImporterService implements IFileImporter {
 ...
 import(file: File): Promise<ImportResult> {
    return new Promise<ImportResult>((resolve, reject) => {
      ...
      reader.onload = (e: any) => {
        try {
          ...
          const originalHeaders = (raw[0] as any[]).map(h => String(h ?? '').trim());
          ...
          const formatted = this.formatData(raw.slice(1), cleanedHeaders, originalHeaders);
   ...
   */
  private formatData(rawData: any[], headers: string[], originalHeaders: string[]): any[] {
  ...
  private toCamelCase(value: string): string {
    ...
    const camel = (first ?? '').toLowerCase() + rest.map(w => this.capitalize(w)).join('');
